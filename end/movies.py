...
movies = db['Movies']
users = db['Users']
#us1 ={'name':'god','email':'god','password': '123' ,'category': 'admin'}
#x = users.insert_one(us1)
...
def login():
    ...
    if users.find({"password":password,"email":email}).count() > 0 :
        iterable = users.find({"password":password,"email":email})
        for user in iterable:
          session['cat']=user['category']
   ...
   if title !=""  and year !="":
        ...
        for movie in iterable:
           ...
           for user in iterable2:
            for j in range(len(movie['comments'])):
             if  session['em'] != movie['comments'][j].get('email') :
               movie['_id'] = None
               dic={'comments':movie['comments'][j]}
               output.append(dic)
        return jsonify(output)  
   else:
...
def adminmenu():
   ...
   for user in iterable:
    if session['cat'] ==  "admin":
          return render_template('4.html')
    else:
     if session['log']=='yes':
          return render_template('3.html',error="You are not admin")
     else:
           return render_template('1.html',error="You are not admin")
